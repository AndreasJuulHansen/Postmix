...
int sequenceConfig1[5] = {1,0,3,3,2};
int sequenceConfig2[5] = {0,3,1,2,0};
int sequenceConfig3[5] = {2,1,0,1,2};
int sequenceConfig4[5] = {0,2,3,1,1};
int sequenceConfig5[5] = {3,1,0,0,2};
...
void start() {
  for (int i = 0; i < 6; i++) {
    if (i != 2){
      digitalWrite(pinsCheckLEDs[i], 0);
      checkLEDs[i] = 0;
    }
  }
  generateSerial();
...
void initializeWires() {
  ...
  if (countEvenNumbers(gameSerial) > 0) {
    wiresTargetConfig[0] = 1;
  }
  else {
    wiresTargetConfig[0] = 0;
  }

    Serial.print("lit sideLEDs = ");
    Serial.print(checkSideLEDs());
  ...
  if (((wiresTargetConfig[0]) + (wiresTargetConfig[1]) + (wiresTargetConfig[2])) % 2 == 1) {
...
int countEvenNumbers(char* input) {
    int count = 0;
    for (int i = 0; i < 8; i++) {
...
bool containsThreeNumbers(char* arr) {
  int count = 0;
  for (int i = 0; i < 8; i++) {
    if (arr[i] >= '0' && arr[i] <= '9') {
      count++;
...
void initializeSequence() {
  ...
  if (togglesTargetConfig[0] == 1 && checkSideLEDs() > 1) {
    for (int i = 0; i < 5; ++i) {
    sequenceTargetColor[i] = sequenceConfig1[i];
    }
  }
  else if (containsLetter(gameSerial, 'H') || containsLetter(gameSerial, 'W')) {
     for (int i = 0; i < 5; ++i) {
    sequenceTargetColor[i] = sequenceConfig2[i];
    }   
  }
  else if (checkSideLEDs() < 1) {
     for (int i = 0; i < 5; ++i) {
    sequenceTargetColor[i] = sequenceConfig3[i];
    }   
  }
  else if (containsThreeNumbers(gameSerial)) {
     for (int i = 0; i < 5; ++i) {
    sequenceTargetColor[i] = sequenceConfig4[i];
    }   
  }
  else {
    for (int i = 0; i < 5; ++i) {
    sequenceTargetColor[i] = sequenceConfig5[i];
    }   
  }
...
// Finds lowest num in char array (Lucas)
int lowestNum(char* arr) {
  int x = 10;
  for (int i = 0; i < 8; i++) {
    for (int j = 9; j >= 0; j--) {
      if (arr[i] == char(48+j)) {
...
void initializeToggles() {
    ...
    else if (checkSideLEDs() >= 2 && highestNum(gameSerial) > 4) {
      for (int i = 0; i < 5; i++) {
...
//check number of switches on in correct solution for toggles:
int togglesON() {
  int x = 0;
  for (int i = 0; i < 5; i++) {
    if (togglesTargetConfig[i] == 1) {
      x++;
    }
  }
  return x;
...
void initializeUltrasonic() {
  //1. If correct solution for toggles includes 3 ON switches. Correct distance is 3 cm
  if (togglesON() > 2) {
    ultrasonicCorrectDistance = 30;

    // 2. If not and there are 2 or 3 numbers in the serial number. The desired distance is 6 cm
  } else if (containsThreeNumbers(gameSerial)) {
...
int sequenceColorNum(int x) {
  int y = 0;
  for (int i = 0; i < 5; i++) {
    if (sequenceTargetColor[i] == x) {
      y++;
    }
  }
  return y;
...
void initializeMotor() {
  if (sequenceColorNum(0) == 2) {
    motorTargetSegment = 8.0;
  }
  else if (sequenceColorNum(0) == 1 && checkSideLEDs() > 1) {
    motorTargetSegment = 5.0;
  }
  else if (sequenceColorNum(1) == 2) {
    motorTargetSegment = 2.0;
  }
  else {
    motorTargetSegment = 10.0;
  }
